...
export default function StudentRepEvents() {
  ...
  useEffect(() => {
    ...
    supabase.from('ct_events').select('id,title,event_date,location,status').eq('institution_id', institutionId).order('event_date', { ascending: false })
      .then(({ data }) => { setEvents(data ?? []); setLoading(false); });
  }, [institutionId]);

  const createEvent = async () => {
    if (!form.title.trim() || !institutionId) return;
    const { data } = await supabase.from('ct_events').insert({ ...form, institution_id: institutionId, status: 'published', category: 'Student Life' }).select('id,title,event_date,location,status').single();
    if (data) { setEvents([data, ...events]); setForm({ title: '', event_date: '', location: '' }); }
  };
